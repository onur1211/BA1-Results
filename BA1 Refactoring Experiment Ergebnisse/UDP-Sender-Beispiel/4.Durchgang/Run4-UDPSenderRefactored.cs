...
public class Sender
{
    ...
    public async Task RunAsync()
    {
        ...
        try
        {
            ...
            bool completed = await SendMessagesAsync(client, endpoint);
    ...
    private async Task<bool> SendMessagesAsync(UdpClient client, IPEndPoint endpoint)
    {
    ...
    private async Task<IPEndPoint?> GetIPEndPointFromHostNameAsync()
    {
        try
        {
            ...
            if (address == null)
            {
                Console.WriteLine($"No {_useIpv6 ? "IPv6" : "IPv4"} address for {_hostName}");
                return null;
            }

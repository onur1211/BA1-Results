...
public class Sender
{
    ...
    public async Task RunAsync()
    {
        ...
        try
        {
            ...
            if (_groupAddress != null)
            {
                ...
                AppDomain.CurrentDomain.ProcessExit += (sender, eventArgs) => client.DropMulticastGroup(IPAddress.Parse(_groupAddress));
            }

            await SendMessagesAsync(client, endpoint);
        }
    ...
    private async Task SendMessagesAsync(UdpClient client, IPEndPoint endpoint)
    {
    ...
    private async Task<IPEndPoint?> GetReceiverIPEndPointAsync()
    {
        try
        {
            return _useBroadcast
                ...
                : _hostName != null
                    ? await GetEndPointFromHostNameAsync()
                    : _groupAddress != null
    ...
    private async Task<IPEndPoint?> GetEndPointFromHostNameAsync()
    {
        try
        {
